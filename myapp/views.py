...
from myapp.models import User, Industry, Product, Institutions, Loan
...
def pledge(request):
    ...
    if request.method == 'GET':
        ...
        if request.user:
            params = get_info(request)
            type = 1
            #  插入借贷表
            loan = Loan()
            # 同时生成一个产品表
            product = Product()
            mortgage_save(loan, product, user, *params, type)

        else:
            messages.add_message(request, messages.INFO, '请先登录！！')
            return redirect(reverse('aqq:login'))
        return redirect(reverse('aqq:buy_success'))
